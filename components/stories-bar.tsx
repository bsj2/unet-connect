...
export function StoriesBar({ currentUserId }: StoriesBarProps) {
  ...
  const fetchStories = async () => {
    try {
      ...
      const myGroupRaw =
        currentUserId && grouped[currentUserId] ? grouped[currentUserId] : null;
  ...
  const handleUploadStory = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.add({
        title: "Error",
        description: "Error uploading story: " + error.message,
        type: "error",
      });
    } finally {
  ...
  const handleDeleteStory = async () => {
    ...
    try {
      ...
      toast.add({
        title: "Error",
        description: "Error deleting story: " + error.message,
        type: "error",
      });
    } finally {
      setIsDeleting(false);
  ...
  if (loading) {
    return (
      <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar px-2">
        <Loader2 className="w-6 h-6 animate-spin text-muted-foreground m-auto" />
      </div>
    );
  }
  ...
  return (
    <>
      <div className="flex gap-4 overflow-x-auto pb-4 pt-2 hide-scrollbar px-2">
        ...
        >
          ...
          <div className="relative">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center p-[2px] ${myStories.length > 0 ? "bg-gradient-to-tr from-yellow-400 to-fuchsia-600" : "bg-muted"}`}
            >
        ...
        <DialogContent className="sm:max-w-[300px]">
          ...
          <div className="flex flex-col gap-2 mt-2">
            <button
              ...
              onClick={() => {
                setShowOptionsDialog(false);
                setActiveStoryGroup(myGroupFull);
              }}
      ...
      <Dialog
        open={!!activeStoryGroup}
        onOpenChange={(open) => !open && setActiveStoryGroup(null)}
      >
              ...
              <div className="absolute top-0 left-0 right-0 p-4 pt-6 flex items-center justify-between bg-gradient-to-b from-black/60 to-transparent z-50 pointer-events-none">
                <div className="flex items-center gap-3 pointer-events-auto">
                  ...
                  <span className="text-white font-semibold text-sm drop-shadow-md">
                    {activeStoryGroup.user?.nombre}{" "}
                    {activeStoryGroup.user?.apellido}
                  </span>
                  <span className="text-white/70 text-xs ml-2">
                    {new Date(
                      activeStoryGroup.stories[currentStoryIndex].created_at,
                    ).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
