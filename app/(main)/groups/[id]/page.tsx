...
export default function GroupPage() {
  ...
  const confirmLeaveGroup = async () => {
    ...
    try {
      ...
      toast.add({
        title: "Error",
        description: "Error leaving group: " + error.message,
        type: "error",
      });
    } finally {
      setActionLoading(false);
  ...
  const confirmDeleteGroup = async () => {
    ...
    try {
      ...
      toast.add({
        title: "Error",
        description: "Error deleting group: " + error.message,
        type: "error",
      });
    } finally {
      setIsDeleting(false);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        updatedAvatarUrl = supabase.storage
          .from("groups")
      ...
      if (bannerFile) {
        ...
        updatedBannerUrl = supabase.storage
          .from("groups")
          .getPublicUrl(filePath).data.publicUrl;
      }
  ...
  if (!group)
    return (
      <div className="text-center py-20 text-red-500">Group not found</div>
    );
          ...
          <div className="mb-2 md:mb-4 flex items-center gap-2">
            {isCreator && (
              <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
                <DialogTrigger
                  data-protected="true"
                  className="flex items-center justify-center gap-2 bg-secondary hover:bg-secondary/80 text-foreground px-4 py-2 rounded-md font-medium text-sm transition-colors border border-border"
                >
                  <Settings size={18} />
                  ...
                  <div className="space-y-6 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <button
                        ...
                        className="flex flex-col items-center justify-center gap-2 p-4 border border-dashed border-border rounded-lg hover:bg-muted/50 transition-colors text-muted-foreground hover:text-foreground"
                      >
                        <Camera size={24} />
                        <span className="text-xs font-medium text-center">
                          {avatarFile ? avatarFile.name : "Change Icon"}
                        </span>
                      </button>
                      <button
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className="w-full bg-muted/50 border border-border rounded-lg p-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary resize-none"
                        rows={4}
                      />
                    </div>
            ...
            {!isCreator &&
              (!isMember ? (
                <button
                  ...
                  className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-2 rounded-md font-medium text-sm transition-colors disabled:opacity-50 min-w-[120px] justify-center"
                >
                  <UserPlus size={18} />
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className="flex items-center justify-center gap-2 bg-secondary hover:bg-destructive hover:text-destructive-foreground text-foreground px-4 py-2 rounded-md font-medium text-sm transition-colors border border-border disabled:opacity-50 group min-w-[120px]"
                >
                  <Users size={18} className="group-hover:hidden" />
                  <LogOut size={18} className="hidden group-hover:block" />
                  <span className="hidden sm:block sm:group-hover:hidden">
                    Joined
                  </span>
          ...
          <div className="md:col-span-1 space-y-4">
            <div className="bg-card border border-border rounded-lg p-5">
              ...
              <p className="text-sm text-muted-foreground">
                Space of discussion for {group.name}. Here, members can share
                posts, ask questions, and collaborate on topics related to this
                group.
              </p>
            </div>
          </div>
            ...
            {isMember || isCreator ? (
              ...
              <div className="bg-card border border-border rounded-lg p-6 text-center mb-6">
                <p className="text-muted-foreground text-sm">
                  Join this group to participate in the discussion and post
                  updates.
                </p>
              </div>
            )}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              able to participate in discussions unless you join again.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              WARNING: This action will permanently delete the group and all its
              posts. This cannot be undone. Are you sure?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
